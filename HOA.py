...
def HOA(SearchAgents, fitness, lowerbound, upperbound, Max_iterations):
    ...
    # Initialization
    X = np.random.rand(SearchAgents.shape[0], dimension) * (upperbound - lowerbound) + lowerbound
    ...
    for t in range(Max_iterations):
        ...
        for i in range(SearchAgents.shape[0] // 2):
            ...
            if T > 0.6:
                X_P2 = X + A * (Dominant_hippopotamus - I2 * MeanGroup)
            else:
                X_P2 = X[i, :] + B * (MeanGroup - Dominant_hippopotamus) if np.random.rand() > 0.5 else np.random.rand(
                    dimension) * (upperbound - lowerbound) + lowerbound
        ...
        for j in range(SearchAgents.shape[0] // 2, SearchAgents.shape[0]):
            ...
            RL = 0.05 * levy(SearchAgents.shape[0], dimension, 1.5)
            if (fit[j] > F_HL).all():
